...
from app.services.translation_service import translate_to_bengali

class AIFarmAdvisor:
    @staticmethod
    def translate_action_to_bengali(action: FarmAdvisorAction) -> FarmAdvisorAction:
        action.category = translate_to_bengali(action.category)
        action.title = translate_to_bengali(action.title)
        action.action = translate_to_bengali(action.action)
        action.why = translate_to_bengali(action.why)
        action.expected_impact = translate_to_bengali(action.expected_impact)
        action.confidence = translate_to_bengali(action.confidence)
        action.urgency = translate_to_bengali(action.urgency)
        action.button_label = translate_to_bengali(action.button_label)
    ...
    def generate_daily_advice() -> AIAdvisorResponse:
        ...
        # Translate recommendation text to Bengali
        recs = [
            AIFarmAdvisor.translate_action_to_bengali(rec)
            for rec in recs
        ]

        # Translate headline, summary and condition
        headline = translate_to_bengali(headline)
        summary = translate_to_bengali(summary)
        condition = translate_to_bengali(condition)

        # Return final advisor response
