...
export const FarmStateProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const aiAdvisor = calculateAIAdvisor(farmState);
  const farmZones = calculateFarmZones(farmState);
  const alerts = calculateAlerts(farmState);
  ...
  const executeAdvisorAction = (actionId: string, actionType: string) => {
    ...
    setTimeout(() => {
      setFarmState((prev) => {
        ...
        if (actionId === 'rec_soil_ec_salinity' || actionType === 'drainage' || actionId.includes('salinity')) {
          updated.ec = 0.75;
        ...
        } else if (actionId === 'rec_nutrient_nitrogen_excess') {
        ...
        } else if (actionId === 'rec_nutrient_nitrogen_low' || actionType === 'fertilize') {
        ...
        } else if (actionId === 'rec_soil_ph_acidic' || actionId === 'rec_soil_ph_alkaline' || actionType === 'amend_ph') {
          updated.ph = 6.5;
          ...
          updated.soil_moisture = Math.max(prev.soil_moisture, 55.0);
        ...
        } else if (actionId === 'rec_water_rain_hold' || actionId === 'hold_irrigation') {
        ...
        } else if (actionId === 'rec_water_irrigate_urgent' || actionId === 'rec_water_irrigate_moderate' || actionType === 'irrigate') {
