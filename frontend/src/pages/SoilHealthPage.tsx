    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-sm text-stone-500 mt-0.5">
          {t('soil.subtitle')} • {tText(farmState.config.soil_type)}
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-white rounded-2xl p-6 border border-stone-200 shadow-sm flex flex-col items-center justify-center text-center">
          ...
          <span className="text-base font-bold text-stone-800 mt-1">{tText(soilHealth.status)}</span>
          <p className="text-xs text-stone-500 mt-1 max-w-xs">{tText(soilHealth.summary)}</p>
        </div>
        ...
        <div className="bg-white rounded-2xl p-6 border border-stone-200 shadow-sm md:col-span-2 flex flex-col justify-between">
          <div>
            <div className="flex items-center justify-between mb-4">
              ...
              <span className={`px-3 py-1 rounded-full text-xs font-bold border ${riskBadgeColor}`}>
                {tText(soilHealth.degradation_risk + ' Risk')}
              </span>
            </div>
            ...
            <div className="space-y-2 mb-4">
              ...
              {soilHealth.degradation_factors.length > 0 ? (
                ...
                <div className="flex items-center space-x-2 p-3 rounded-lg bg-emerald-50 text-emerald-800 text-xs font-semibold">
                  ...
                  <span>{tText("No active degradation factors detected. Soil physical & chemical balances are optimal.")}</span>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="pt-3 border-t border-stone-100 flex items-center justify-between text-xs text-stone-500">
            <span>{tText("Methodology: Multi-Criteria Agronomic Telemetry")}</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-2xl p-6 border border-stone-200 shadow-sm">
          <div className="flex items-center justify-between mb-4">
            ...
            <span className="text-xs text-stone-400 font-medium">{tText("Optimal: 50–85 mg/kg")}</span>
          </div>
          ...
          <div className="grid grid-cols-3 gap-2 mt-4 text-center text-xs">
            {npkChartData.map((item) => (
              <div key={item.name} className="bg-stone-50 p-2.5 rounded-xl border border-stone-100">
                <span className="text-stone-500 block font-medium">{tText(item.name.split(' ')[0])}</span>
        ...
        <div className="bg-white rounded-2xl p-6 border border-stone-200 shadow-sm flex flex-col justify-between">
          <div>
            <h3 className="text-base font-bold text-stone-900 mb-4 flex items-center space-x-2">
              ...
              <span>{tText("Soil Physical & Chemical Telemetry")}</span>
            </h3>
              ...
              {soilHealth.nutrients.map((nutr) => (
                ...
                >
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      <div className="text-xs font-bold text-stone-900">{tText(nutr.name)}</div>
                      <div className="text-[11px] text-stone-500">
                        {tText("Target:")} {nutr.optimal_min}–{nutr.optimal_max} {nutr.unit}
                      </div>
                    </div>
                  </div>
          ...
          <div className="mt-4 p-3 bg-stone-100/70 rounded-xl text-[11px] text-stone-500 flex items-start space-x-2">
            ...
            <span>
              <strong>{tText("Scientific Note:")}</strong> {tText("IoT NPK & optical pH telemetry provide indicative field readings for immediate decision support. Periodic government soil lab testing is recommended for baseline calibration.")}
            </span>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl p-6 border border-stone-200 shadow-sm">
        <h3 className="text-base font-bold text-stone-900 mb-4 flex items-center space-x-2">
          ...
          <span>{tText("Soil Health & Moisture Trend")}</span>
        </h3>
        <div className="h-64 w-full">
          <ResponsiveContainer width="100%" height="100%">
