      ...
      <div className="bg-white rounded-2xl p-5 border border-stone-200 shadow-sm flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs sm:text-sm text-stone-500 mt-0.5">
            {farmState.config.farm_name} • {tText(farmState.config.crop_type)} ({farmState.config.crop_variety}) • {farmState.config.area_acres} {t('common.acres')} • {tText(farmState.config.growth_stage + ' Stage')}
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
        ...
        <Link to="/app/sustainability" className="block">
          <MetricCard
            ...
            statusText={tText(`Rating: ${sustainability.sustainability_rating}`)}
        ...
        <Link to="/app/market" className="block">
          <MetricCard
            ...
            unit="/ Q"
            subtitle={tText(marketIntelligence.best_market_name.split(' ')[0] + ' Mandi')}
            icon={Store}
