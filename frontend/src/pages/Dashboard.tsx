      ...
      <div className="bg-white rounded-2xl p-5 border border-stone-200 shadow-sm flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs sm:text-sm text-stone-500 mt-0.5">
            {farmState.config.farm_name} • {farmState.config.crop_type} ({farmState.config.crop_variety}) • {farmState.config.area_acres} {t('common.acres')} • {farmState.config.growth_stage} Stage
          </p>
        </div>
        ...
        <div className="flex items-center space-x-3 bg-stone-50 p-3 rounded-xl border border-stone-100 text-xs text-stone-700">
          ...
          <div>
            <div className="font-bold text-stone-900 text-sm">
              {farmState.air_temperature}°C • {farmState.weather_condition}
            </div>
            <div className="text-stone-500 text-[11px]">
              Rain Chance: <span className="font-semibold text-blue-600">{farmState.rain_probability}%</span> ({farmState.expected_rainfall} mm)
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
        ...
        <Link to="/app/soil-health" className="block">
          <MetricCard
            ...
            unit="/ 100"
            subtitle={soilHealth.status}
            icon={Layers}
            iconColor="text-emerald-700"
            statusText={soilHealth.degradation_risk + ' Risk'}
        ...
        <Link to="/app/irrigation" className="block">
          <MetricCard
            ...
            unit="%"
            subtitle={`Pump: ${irrigation.pump_status}`}
            icon={Droplets}
            iconColor="text-blue-600"
            statusText={irrigation.action}
        ...
        <Link to="/app/crop" className="block">
          <MetricCard
            ...
            unit="%"
            subtitle={`${cropAnalysis.expected_yield_quintals_per_acre} Q / Acre`}
            icon={Sprout}
            iconColor="text-green-600"
            statusText={cropAnalysis.growth_stage}
        ...
        <Link to="/app/economics" className="block">
          <MetricCard
            ...
            subtitle={`+₹${economics.net_profit_gain_inr.toLocaleString()} gain`}
            icon={IndianRupee}
            iconColor="text-amber-600"
            statusText={`Save ${economics.input_cost_reduction_percent}%`}
            statusColor="emerald"
          />
        </Link>
        ...
        <Link to="/app/sustainability" className="block">
          <MetricCard
            ...
            value={soilHealth.degradation_risk}
            subtitle={`Saved: ${sustainability.water_saved_liters.toLocaleString()} L`}
            icon={ShieldAlert}
