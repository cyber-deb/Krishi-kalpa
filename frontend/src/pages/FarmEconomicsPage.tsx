    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-sm text-stone-500 mt-0.5">
          {t('economics.subtitle')} • {farmState.config.area_acres} {t('common.acres')} ({tText(farmState.config.crop_type)})
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 sm:gap-6">
        ...
        <div className="bg-white rounded-2xl p-5 border border-stone-200 shadow-sm">
          ...
          <p className="text-xs text-stone-500">
            {tText("Higher projected return from balanced inputs & yield protection")}
          </p>
        </div>
        ...
        <div className="bg-white rounded-2xl p-5 border border-stone-200 shadow-sm">
          ...
          <p className="text-xs text-stone-500">
            {economics.input_cost_reduction_percent}% {tText("reduction in wasted fertilizers & energy")}
          </p>
        </div>
        ...
        <div className="bg-white rounded-2xl p-5 border border-stone-200 shadow-sm">
          ...
          <p className="text-xs text-stone-500">
            {tText("Debt Coverage Ratio:")} {economics.debt_service_ratio}x
          </p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl p-6 border border-stone-200 shadow-sm">
        ...
        <div className="bg-stone-50 p-4 rounded-xl border border-stone-100 mb-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs mb-3">
            <div>
              <span className="text-stone-400 block font-medium">{tText("Estimated Crop Loan / KCC Debt:")}</span>
              <span className="text-base font-bold text-stone-900 font-serif">₹{economics.outstanding_debt_inr.toLocaleString()}</span>
            </div>
            <div>
              <span className="text-stone-400 block font-medium">{tText("Annual Net Surplus Capacity:")}</span>
              <span className="text-base font-bold text-emerald-700 font-serif">₹{(rec.estimated_net_profit * 2).toLocaleString()}</span>
            </div>
          </div>
          <p className="text-xs text-stone-700 leading-relaxed">
            <strong>{tText("Financial Guidance:")}</strong> {tText(economics.financial_guidance)}
          </p>
        </div>
        ...
        <div className="text-[11px] text-stone-400 flex items-start space-x-2">
          ...
          <span>{tText(economics.disclaimer)}</span>
        </div>
      </div>
    </div>
  );
