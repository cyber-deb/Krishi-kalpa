    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-sm text-stone-500 mt-0.5">
          {t('crop.subtitle')} • {tText(farmState.config.crop_type)} ({farmState.config.crop_variety})
        </p>
      </div>
        ...
        <div className="md:col-span-2 flex flex-col justify-between p-2">
          <div>
            ...
            <h3 className="text-xl font-bold text-stone-900 font-serif mb-2">
              {tText(cropAnalysis.growth_stage + ' Stage')} ({tText(farmState.config.crop_type)})
            </h3>
          ...
          <div className="grid grid-cols-2 gap-3 mt-4 pt-3 border-t border-stone-100 text-xs">
            <div>
              <span className="text-stone-400 font-medium block">{tText("Sowing Date:")}</span>
            ...
            <div>
              ...
              <span className="font-bold text-stone-800">~{cropAnalysis.estimated_days_to_harvest} {tText("Days")}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-white rounded-2xl p-5 border border-stone-200 shadow-sm">
          <div className="flex items-center justify-between mb-2">
            <span className="text-xs font-bold text-stone-400 uppercase tracking-wider">{tText("Moisture Stress")}</span>
          ...
          <div className="text-[11px] text-stone-500">
            {tText("Soil moisture:")} {farmState.soil_moisture}%
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl p-5 border border-stone-200 shadow-sm">
          <div className="flex items-center justify-between mb-2">
            <span className="text-xs font-bold text-stone-400 uppercase tracking-wider">{tText("Nutrient Stress")}</span>
        ...
        <div className="bg-white rounded-2xl p-5 border border-stone-200 shadow-sm">
          <div className="flex items-center justify-between mb-2">
            <span className="text-xs font-bold text-stone-400 uppercase tracking-wider">{tText("Heatwave Stress")}</span>
          ...
          <div className="text-[11px] text-stone-500">
            {tText("Air temp:")} {farmState.air_temperature}°C
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl p-5 border border-stone-200 shadow-sm">
          <div className="flex items-center justify-between mb-2">
            <span className="text-xs font-bold text-stone-400 uppercase tracking-wider">{tText("Fungal Disease Risk")}</span>
          ...
          <div className="text-[11px] text-stone-500">
            {tText("Humidity:")} {farmState.humidity}%
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl p-6 border border-stone-200 shadow-sm">
        <h3 className="text-base font-bold text-stone-900 mb-4 flex items-center space-x-2">
          ...
          <span>{tText("Yield Projection & Loss Mitigation")}</span>
        </h3>

        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-center">
          <div className="bg-stone-50 p-4 rounded-xl border border-stone-100">
            <span className="text-xs font-medium text-stone-500 block">{tText("Baseline Genetic Potential")}</span>
            <span className="text-2xl font-extrabold text-stone-900 font-serif">24.5 {tText("Q / Acre")}</span>
          </div>

          <div className="bg-stone-50 p-4 rounded-xl border border-stone-100">
            <span className="text-xs font-medium text-stone-500 block">{tText("Expected Model Harvest")}</span>
          ...
          <div className="bg-stone-50 p-4 rounded-xl border border-stone-100">
            <span className="text-xs font-medium text-stone-500 block">{tText("Yield Penalty Risk")}</span>
