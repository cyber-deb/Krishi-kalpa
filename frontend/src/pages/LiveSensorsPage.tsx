...
export const LiveSensorsPage: React.FC = () => {
  ...
  const { farmState, updateCustomSensor, toggleFarmMode } = useFarmState();
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-2xl font-extrabold text-stone-900 font-serif tracking-tight">
            {t('nav.live_sensors')} Telemetry
          </h2>
          <p className="text-sm text-stone-500 mt-0.5">
            Root zone IoT sensor cluster & microclimate weather telemetry
          </p>
        </div>
        ...
        >
          ...
          <span>{isDemo ? 'Showing Simulated Telemetry (Click for Live)' : 'Listening for ESP32 Hardware'}</span>
        </button>
      </div>
      ...
      <div className="bg-white rounded-2xl p-5 border border-stone-200 shadow-sm grid grid-cols-2 sm:grid-cols-4 gap-4 text-xs">
        <div className="flex items-center space-x-3">
          ...
          <div>
            <div className="text-stone-400 font-bold uppercase tracking-wider text-[10px]">Device ID</div>
        ...
        <div className="flex items-center space-x-3">
          ...
          <div>
            <div className="text-stone-400 font-bold uppercase tracking-wider text-[10px]">Signal Strength</div>
            <div className="font-extrabold text-stone-900">{farmState.signal_strength} dBm (Good)</div>
          </div>
        </div>
        ...
        <div className="flex items-center space-x-3">
          ...
          <div>
            <div className="text-stone-400 font-bold uppercase tracking-wider text-[10px]">Battery Level</div>
            <div className="font-extrabold text-stone-900">{farmState.battery}% (Solar Charged)</div>
          </div>
        </div>
        ...
        <div className="flex items-center space-x-3">
          ...
          <div>
            <div className="text-stone-400 font-bold uppercase tracking-wider text-[10px]">Last Sync</div>
            <div className="font-extrabold text-stone-900">Just Now (Real-time)</div>
          </div>
        </div>
      </div>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-6">
          <div>
            <h3 className="text-base font-bold text-stone-900">
              Telemetry Trend (Last 12 Hours)
            </h3>
            <p className="text-xs text-stone-500">
              Observing microclimate and root zone dynamics over time
            </p>
          </div>
        ...
        <div className="h-72 w-full">
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={historyData} margin={{ top: 10, right: 10, left: -15, bottom: 0 }}>
              ...
              <Tooltip contentStyle={{ backgroundColor: '#1c1917', color: '#fff', borderRadius: '8px', fontSize: '12px' }} />
              <Legend wrapperStyle={{ fontSize: '12px', paddingTop: '10px' }} />
              <Line
                type="monotone"
                dataKey={selectedParam}
                stroke={selectedParam === 'soil_moisture' ? '#0284c7' : selectedParam === 'nitrogen' ? '#16a34a' : '#f59e0b'}
                strokeWidth={3}
                dot={{ r: 4 }}
                activeDot={{ r: 6 }}
                name={selectedParam === 'soil_moisture' ? 'Moisture (%)' : selectedParam === 'nitrogen' ? 'Nitrogen (mg/kg)' : 'Temperature (°C)'}
              />
            </LineChart>
          </ResponsiveContainer>
        </div>
