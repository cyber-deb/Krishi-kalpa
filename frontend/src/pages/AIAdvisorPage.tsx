    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-sm text-stone-500 mt-0.5">
          {t('advisor.subtitle')} • {tText("Connected Agricultural Inference Engine")}
        </p>
      </div>
      ...
      <div className="bg-gradient-to-r from-agri-900 to-stone-900 text-white rounded-2xl p-6 shadow-md border border-agri-800">
        <div className="flex items-center space-x-2 text-xs font-bold text-amber-400 uppercase tracking-wider mb-1">
          ...
          <span>{tText("Farm Health Evaluation")}</span>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        {filteredRecs.length > 0 ? (
          ...
          <div className="col-span-2 text-center py-12 bg-white rounded-2xl border border-stone-200">
            ...
            <h4 className="text-lg font-bold text-stone-900">{tText("No Pending Actions in this Category")}</h4>
            <p className="text-xs text-stone-500 mt-1">{tText("Farm parameters are operating smoothly within target thresholds.")}</p>
          </div>
        )}
      </div>
      ...
      <div className="bg-stone-100 p-4 rounded-2xl border border-stone-200 text-xs text-stone-600 flex items-start space-x-3">
        ...
        <div>
          <span className="font-bold text-stone-900 block mb-0.5">{tText("Philosophy: Helping Farmers Act")}</span>
          <span>
            {tText("Every recommendation is calculated strictly from live root zone telemetry, soil chemical balances, and meteorological radars. Actions taken update the farm state and prevent unnecessary input expenditures.")}
          </span>
        </div>
      </div>
    </div>
