    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-sm text-stone-500 mt-0.5">
          {t('market.subtitle')} • {tText(farmState.config.crop_type)} ({farmState.config.crop_variety})
        </p>
      </div>
      ...
      <div className="bg-gradient-to-r from-agri-900 to-stone-900 text-white rounded-2xl p-6 shadow-md border border-agri-800 flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          ...
          <h3 className="text-2xl sm:text-3xl font-bold font-serif text-white">
            {tText(marketIntelligence.best_market_name)}
          </h3>
        ...
        <div className="flex items-center space-x-4 bg-stone-800/80 p-4 rounded-xl border border-stone-700 self-start md:self-auto">
          <div>
            ...
            <div className="text-[11px] text-stone-400">
              {tText("Total Revenue:")} <span className="font-bold text-white">₹{marketIntelligence.total_estimated_revenue.toLocaleString()}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl p-6 border border-stone-200 shadow-sm">
        <h3 className="text-base font-bold text-stone-900 mb-4 flex items-center space-x-2">
          ...
          <span>{tText("Regional APMC Mandi Price Discovery & Logistics")}</span>
        </h3>
          ...
          <table className="w-full text-left text-xs sm:text-sm">
            <thead>
              <tr className="border-b border-stone-200 text-stone-400 font-bold uppercase tracking-wider text-[11px]">
                <th className="py-3 px-4">{tText("APMC Mandi Yard")}</th>
                ...
                <th className="py-3 px-4">{tText("APMC Cess & Fees")}</th>
                <th className="py-3 px-4 text-emerald-700 font-extrabold">{t('market.net_realization')}</th>
                <th className="py-3 px-4 text-right">{tText("Trend / Status")}</th>
              </tr>
            </thead>
              ...
              {marketIntelligence.markets.map((m) => (
                ...
                >
                  <td className="py-4 px-4">
                    <div className="font-bold text-stone-900">{tText(m.mandi_name)}</div>
                    <div className="text-[11px] text-stone-400 font-normal">{tText(m.district + ' District')}</div>
                  </td>
      ...
      <div className="bg-stone-100 p-4 rounded-2xl border border-stone-200 text-xs text-stone-600 flex items-start space-x-3">
        ...
        <div>
          <span className="font-bold text-stone-900 block mb-0.5">{tText("Formula: Net In-Hand Realization")}</span>
          <span>
            {tText("Net Realization (₹/Q) = Mandi Modal Selling Price - Road Freight Transport Cost - APMC Cess & Weighment Fees. Always select the market yielding the highest net return rather than simply the highest gross listing price.")}
          </span>
        </div>
      </div>
    </div>
